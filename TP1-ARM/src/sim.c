...
void process_instruction() {
    ...
    uint32_t instruction =  (CURRENT_STATE.PC);

    printf("Executing instruction: 0b%032b\n", instruction);

    uint32_t opcodes[4];
    opcodes[0] = (instruction >> 21) & 0b11111111111; // Extraer opcode21
    opcodes[1] = (instruction >> 22) & 0b11111111111; // Extraer opcode22
    opcodes[2] = (instruction >> 26) & 0b111111;      // Extraer opcode26
    opcodes[3] = (instruction >> 24) & 0b111111;      // Extraer opcode24

    uint32_t opcode21 = opcodes[0];
    uint32_t opcode22 = opcodes[1];
    uint32_t opcode26 = opcodes[2];
    ...
    for (int i = 0; i < 4; i++) {
        switch (opcodes[i]) {
            ...
            case 0b1101001101: // LSL (immediate)
            ...
            case 0b1101001101: // LSR (immediate)
                uint32_t rd = instruction & 0b11111; // Extract bits 0 to 4
                uint32_t rn = (instruction >> 5) & 0b11111; // Extract bits 5 to 9
                uint32_t immr = (instruction >> 16) & 0b111111; // Extract bits 16 to 21
            ...
            case 0b11111000000: // STUR Xn, [Xn, #imm]
                ...
                uint64_t address;
                if (Rn == 31) {
                    if (CURRENT_STATE.SP % 16 != 0) {
                        printf("Error: Stack Pointer misaligned!\n");
                        exit(1); // O generar una excepción
                    address = CURRENT_STATE.SP;
                } 
                }
                else {
                ...
                // Calcular la dirección final con el offset
                address += imm9;
            ...
            default:
                printf("opcode21: 0b%011b\n", opcode21);
                printf("opcode22: 0b%011b\n", opcode22);
                printf("opcode26: 0b%06b\n", opcode26);
                printf("opcode24: 0b%06b\n", opcode24);
                break;
        }
    }
}